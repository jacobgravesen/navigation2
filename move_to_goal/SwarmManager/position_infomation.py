...
class PositionInfo(Node):
    ...
    def create_goals(self, robot_list, goal_list, goals, norobot):
        goal_poses = []
        for k in range(len(goals[norobot - 1])):
            goal_list[f"robot{norobot}"].header.frame_id = 'map'
            goal_list[f"robot{norobot}"].header.stamp = robot_list[f"robot{norobot}"].get_clock().now().to_msg()
            goal_list[f"robot{norobot}"].pose.position.x = goals[norobot - 1][k][0]
            goal_list[f"robot{norobot}"].pose.position.y = goals[norobot - 1][k][1]
            goal_list[f"robot{norobot}"].pose.orientation.w = 1.0
            goal_poses.append(goal_list[f"robot{norobot}"])
        return goal_poses
